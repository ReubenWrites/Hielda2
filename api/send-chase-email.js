...
function buildEmail(invoice, profile, stage, dl, interest, pen, total) {
  ...
  const bodies = {
    reminder_1: `<p>Dear ${invoice.client_name},</p><p>This is a friendly reminder that invoice <strong>${invoice.ref}</strong> for <strong>${fmt(invoice.amount)}</strong> is due on <strong>${formatDate(invoice.due_date)}</strong>.</p><p>Please ensure payment is made by the due date.</p>${payBlock}<p>Kind regards,<br/>${fromName}</p>`,
    ...
    first_chase: `<p>Dear ${invoice.client_name},</p><p>Invoice <strong>${invoice.ref}</strong> for <strong>${fmt(invoice.amount)}</strong> was due on <strong>${formatDate(invoice.due_date)}</strong> and remains unpaid. Interest at ${RATE}% p.a. is now accruing.</p><p>Please arrange payment immediately.</p>${payBlock}<p>Regards,<br/>${fromName}</p>`,
