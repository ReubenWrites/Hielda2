...
// Unsubscribe lives on this endpoint (?action=unsubscribe) instead of its
// own function — the Vercel Hobby plan caps deployments at 12 serverless
// functions. GET = footer link click; POST = RFC 8058 one-click from the
// mail client (form-encoded body, so it must be routed before any JSON
// parsing).
async function handleUnsubscribe(req, res) {
  ...
  if (!email || !token) {
    return res.status(400).send(unsubscribePage('Link not valid', 'This unsubscribe link is missing some information. Please use the link from the bottom of the email.'))
  }
  ...
  if (!lead || !lead.unsubscribe_token || lead.unsubscribe_token !== token) {
    return res.status(400).send(unsubscribePage('Link not valid', 'This unsubscribe link doesn\'t match our records. Please use the link from the bottom of the most recent email.'))
  }
  ...
  return res.status(200).send(unsubscribePage("You're unsubscribed", 'You won\'t receive any more emails from this series. The free calculator, letter generator and guides remain free to use any time.'))
}
